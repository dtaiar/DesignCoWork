...
export default async function TopicPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-3 px-4 py-4">
        {(captures ?? []).length === 0 && (
          <p className="text-sm text-muted-foreground">
            Ask something, paste a Figma link, or drop a requirement below to get started.
          </p>
        )}
          ...
          return (
            ...
            >
              ...
              {capture.enrichment && (
                <div
                  className={cn(
                    "mt-3 flex items-start gap-2 border-t border-border pt-3 text-sm leading-relaxed text-muted"
                  )}
                >
      ...
      <form
        action={boundAddCapture}
        className="sticky bottom-0 mx-auto flex w-full max-w-2xl items-end gap-2 border-t border-border bg-background p-4"
      >
        <Textarea
          name="content"
          rows={2}
          placeholder="Ask something, paste a Figma link, or add a requirement..."
        />
